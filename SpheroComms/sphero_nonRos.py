...
#
# Modified version of sphero.py ommitting use of ROS framework 
#
...
import sphero_driver_modified as sphero_driver

import argparse

def changeVel(pipe,vel):
    vel_command=str(vel[0])+","+str(vel[1])
    pipe.write(vel_command + "\n")
    pipe.flush()
    return
def changeCol(pipe,rgbVect):
    rgb = str(rgbVect[0])+","+str(rgbVect[1])+","+str(rgbVect[2])
    pipe.write(rgb + "\n")
    pipe.flush()

def connectToSpheroPipe(target_name):
    path = "./pipes/"+target_name + ".fifo"
    print(path)
    return open(path,"w",1)
    
def connectToSphero(target_name,target_addr,port):
    s = SpheroNode(target_name, target_addr, port)
    while not (s.is_connected):
        s.start()
        time.sleep(1)
    print("Connected to "+target_name + " via connect function")
    return s
...
class SpheroNode(object):
    ...
    def start(self):
        try:
            ...
            print ("Failed to connect to Sphero, retrying")
            #sys.exit(1)
            return False
        #setup streaming    
        ...
        self.robot.set_rgb_led(0,255,0,0,False)
    ...
    def set_color(self, msg):
    	# msg is a 1x3 array [r,g,b] values
        if self.is_connected:
...
if __name__ == '__main__':
    ...
    while not (s.is_connected):
    	s.start()
    print("Connected to "+args.target_name)
    s.cmd_vel([70,0])
