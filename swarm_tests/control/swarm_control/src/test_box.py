...
if __name__=="__main__":
    ...
    sphero1 = str(sys.argv[1])

    # Initiate box test node
    rospy.init_node(sphero1+"_box_test")

    # publish to topics
    pub1 = rospy.Publisher(sphero1 +'/cmd_vel', geometry_msgs.msg.Twist, queue_size=1, latch=True)

    rate = rospy.Rate(10) # set publish rate

    # subscribe to odometry data
    rospy.Subscriber(sphero1+'/odom', Odometry, callback=save_odometry)
    ...
    travel_ind = 0;
    while time.time()-start < 60: # not rospy.is_shutdown():
        pub1.publish(vel_msgs[travel_ind])
