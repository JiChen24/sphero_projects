...
if __name__=="__main__":
    ...
    obj = swarmParams.sysParams(3)

    waypoints=numpy.array([ [1.0,1.5] ])
    ...
    # publish to topics
    pub = rospy.Publisher(sphero_name+'/cmd_vel', geometry_msgs.msg.Twist, queue_size=5, latch=True)
    rate = rospy.Rate(5) # set publish rate

    # subscribe to odometry data
    rospy.Subscriber(sphero_name+'/odom', Odometry, callback=save_odometry)
    
    #Initialize the sphero as a vicon tracked object
    a1 = vt.ViconTrackerPoseHandler(None, None, "",51018, vtName)

    a2 = vt.ViconTrackerPoseHandler(None, None, "",51017, 'Sphero1')
    a3 = vt.ViconTrackerPoseHandler(None, None, "",51016, 'Sphero2')

    obj.initPose()
    obj.goalPose()
    obj.neighborRadius()

    obj.goalPose[0,2] = waypoints[0,0]
    obj.goalPose[1,2] = waypoints[0,1]
...
#index of first goal is 0, last is num_waypoints-1
    ...
    while (dist2goal > 0.2): # not at goal point
        ...
        statesNeighbor = numpy.array([[loc1[0],loc2[0]],[loc1[1],loc2[1]]])
        statesNeighbor = numpy.matrix(statesNeighbor)

        poseAll = numpy.array([[loc1[0],loc2[0],loc[0]],[loc1[1],loc2[1],loc[1]]])
        poseAll = numpy.matrix(poseAll)
        obj.getCurrentPose(poseAll)

        indNeighbor = numpy.array([0,1])
        ...
        v_vect_V_actual = actualController(poseAll[:,2],2,statesNeighbor,indNeighbor,obj,MAP)
        print "actual_vel="
