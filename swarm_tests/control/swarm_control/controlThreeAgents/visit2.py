...
if __name__=="__main__":
    ...
    obj.goalPose[0,1] = waypoints[0,0]
    obj.goalPose[1,1] = waypoints[0,1]
...
#index of first goal is 0, last is num_waypoints-1
    ...
    while (dist2goal > 0.2): # not at goal point
        ...
        statesNeighbor = numpy.array([[loc1[0],loc3[0]],[loc1[1],loc3[1]]])
        statesNeighbor = numpy.matrix(statesNeighbor)

        poseAll = numpy.array([[loc1[0],loc[0],loc3[0]],[loc1[1],loc[1],loc3[1]]])
        poseAll = numpy.matrix(poseAll)
        obj.getCurrentPose(poseAll)

        indNeighbor = numpy.array([0,2])
        ...
        v_vect_V_actual = actualController(poseAll[:,1],1,statesNeighbor,indNeighbor,obj,MAP)
        print "actual_vel="
