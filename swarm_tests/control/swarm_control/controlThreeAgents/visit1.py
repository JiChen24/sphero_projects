...
if __name__=="__main__":
    ...
    b1 = vt.ViconTrackerPoseHandler(None, None, "",51020, 'Sphero4')
    b2 = vt.ViconTrackerPoseHandler(None, None, "",51021, 'Sphero5')
    b3 = vt.ViconTrackerPoseHandler(None, None, "",51022, 'Sphero6')
...
#index of first goal is 0, last is num_waypoints-1
    ...
    while (dist2goal > 0.2): # not at goal point
        ...
        statesNeighbor = numpy.array([[loc2[0],loc3[0]],[loc2[1],loc3[1]]])
        statesNeighbor = numpy.matrix(statesNeighbor)

        poseAll = numpy.array([[loc[0],loc2[0],loc3[0]],[loc[1],loc2[1],loc3[1]]])
        poseAll = numpy.matrix(poseAll)
        obj.getCurrentPose(poseAll)

        indNeighbor = numpy.array([1,2])
